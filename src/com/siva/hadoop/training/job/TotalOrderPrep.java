...
public class TotalOrderPrep {
	...
	public static void apply(Job job) throws IOException, ClassNotFoundException {
		...
		// by the real job, but only use 1 reducer for this job (maps output
		// very
		// little)
		sample_job.getConfiguration().setInt(ARG_SAMPLESIZE, job.getNumReduceTasks());
		sample_job.setNumReduceTasks(1);

		// Make this job's output a temporary filethe input file for the real
		// job's
	...
	/**
	 * An input format that makes a record reader which passes few KV pairs to
	 * the mapper
	 */
	public static class SamplingInputFormat extends FileInputFormat {
		...
		public RecordReader createRecordReader(InputSplit split, TaskAttemptContext context) throws IOException, InterruptedException {
			...
			InputFormat realif = (InputFormat) ReflectionUtils.newInstance(conf.getClass(ARG_INPUTFORMAT, TextInputFormat.class), conf);
	...
	public static class SamplingRecordReader<K, V> extends RecordReader {
		...
		public void initialize(InputSplit split, TaskAttemptContext context) throws IOException, InterruptedException {
			...
			double seen = 0.0;
			/*
			 * int samples = context.getConfiguration().getInt(ARG_MAP_SAMPLES,
			 * MAP_SAMPLES); while( real.nextKeyValue() ) { if(
			 * key_buffer.size() < samples ){
			 * key_buffer.add(WritableUtils.clone(real.getCurrentKey(), conf));
			 * val_buffer.add(WritableUtils.clone(real.getCurrentValue(),
			 * conf)); }
			 * 
			 * else if( Math.random() = key_buffer.size() ) return false;
			 * 
			 * ++current; return true;
			 */
		}
	...
	public static class PartitioningReducer extends Reducer {
		...
		public void run(Context context) throws IOException, InterruptedException {
			...
			// is protected...
			long N = context.getCounter("org.apache.hadoop.mapred.Task$Counter", "MAP_OUTPUT_RECORDS").getValue();
