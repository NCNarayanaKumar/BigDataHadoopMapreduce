...
@Description(name = "md5sum", value = "_FUNC_(string|byte, string|byte, ...) - computes the MD5 sum of the inputs.  Returns the hash in hexidecimal format.", extended = "_FUNC_(string|byte, string|byte, ...) - computes the MD5 sum of the inputs.  Returns the hash in hexidecimal format.\n\nExample: _FUNC_('foo', 'bar') = 3858f62230ac3c915f300c664312c63f")
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (objectInspector.getCategory().equals(PRIMITIVE)) {
				...
				if (cat.equals(STRING) || cat.equals(BINARY)) {
					...
					throw new UDFArgumentTypeException(i, String.format("Expected a string or byte argument, received a %s at index %d",
							oi.getPrimitiveCategory(), i));
				}
			} else {
				throw new UDFArgumentTypeException(i, String.format("Expected a string or byte argument, received a %s at index %d",
						objectInspector.getCategory(), i));
			}
		}
